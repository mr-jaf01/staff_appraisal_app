...
Class Action {
	...
	function signup(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("INSERT IGNORE INTO users set $data");
	...
	function update_user(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("INSERT IGNORE INTO {$type[$_SESSION['login_type']]} set $data");
		}else{
	...
	function save_system_settings(){
		...
		if($chk->num_rows > 0){
			...
			$save = $this->db->query("INSERT IGNORE INTO system_settings set $data");
		}
		if($save){
			foreach($_POST as $k => $v){
				if(!is_numeric($k)){
	...
	function save_department(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT IGNORE INTO department_list set $data");
		}else{
	...
	function save_designation(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT IGNORE INTO designation_list set $data");
		}else{
	...
	function save_employee(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("INSERT IGNORE INTO employee_list set $data");
		}else{
	...
	function save_evaluator(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("INSERT IGNORE INTO evaluator_list set $data");
		}else{
	...
	function save_task(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT IGNORE INTO task_list set $data");
		}else{
	...
	function save_progress(){
		...
		if(!isset($is_complete))
			$data .= ", is_complete=0 ";
		if(empty($id)){
			$save = $this->db->query("INSERT IGNORE INTO task_progress set $data");
		}else{
	...
	function save_evaluation(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT IGNORE INTO ratings set $data");
		}else{
